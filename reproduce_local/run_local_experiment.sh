...
# ============== Embedding 服务配置 ==============
# 设置Embedding主机和端口
# 支持单端口: EMBEDDING_PORTS="30151"
# 支持多端口负载均衡: EMBEDDING_PORTS="30151,30152,30153,30154,30155,30156,30157,30158"
EMBEDDING_HOST="10.0.4.178"
EMBEDDING_PORTS="30151,30152,30153,30154,30155,30156,30157,30158"
# EMBEDDING_PORTS="30151"
export EMBEDDING_HOST
export EMBEDDING_PORTS

# 是否使用远程Embedding服务 (true/false)
USE_REMOTE_EMBEDDING="false"
export USE_REMOTE_EMBEDDING
echo "🌐 OSS服务配置:"
echo "  主机: $OSS_HOST"
if [[ "$OSS_PORTS" == *","* ]]; then
...
echo "🔮 Embedding服务配置:"
echo "  主机: $EMBEDDING_HOST"
if [[ "$EMBEDDING_PORTS" == *","* ]]; then
    port_count=$(echo "$EMBEDDING_PORTS" | tr ',' '\n' | wc -l)
    echo "  负载均衡: 启用 ($port_count 个端口)"
    echo "  端口列表: $EMBEDDING_PORTS"
else
    echo "  单一服务: $EMBEDDING_HOST:$EMBEDDING_PORTS"
fi
echo "  远程模式: $USE_REMOTE_EMBEDDING"
...
# 并发配置（根据OSS服务数量调整）
export MAX_ASYNC=80  # LLM并发
export EMBEDDING_FUNC_MAX_ASYNC=10  # Embedding并发
echo "⚡ 并发配置:"
